...
import com.modularwarfare.utility.DiscordRPCLoader;
...
public class ClientEventHandler {
    ...
    public void onGuiLaunch(GuiOpenEvent event) {
        if (event.getGui() instanceof GuiMainMenu) {
            DiscordRPCLoader.updatePresence("Dans les menus");
        }
        else if (event.getGui() instanceof GuiIngameMenu) {
            DiscordRPCLoader.updatePresence("En pleine pause");
        }
        else if (event.getGui() instanceof GuiInventory) {
            if (mc.isSingleplayer()) {
                DiscordRPCLoader.updatePresence("Entrain de cacher l'OR");
            }
            else if (mc.world !=null && mc.getConnection() != null) {
                DiscordRPCLoader.updatePresence("Sur la ligne de front");

            }
        }
        if (event.getGui() instanceof GuiMainMenu) {
            //event.setGui((GuiScreen) (new com.modularwarfare.client.gui.GuiMainMenu()).getGuiScreen());
        }
        if (ModConfig.INSTANCE.general.customInventory) {
            ...
            //if (event.getGui() != null && event.getGui().getClass() == GuiChest.class) {
            //    final EntityPlayer player = Minecraft.getMinecraft().player;
            //    if (!player.isCreative()) {
            //        event.setCanceled(true);
            //        ModularWarfare.NETWORK.sendToServer(new PacketOpenGui(1));
            //    }
            //}
        }
    }
