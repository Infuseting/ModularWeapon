...
public class ItemCMS extends Item implements IHasModel {
    public ItemCMS(String name)
    {
        ...
        if (entity instanceof EntityPlayer)
        {
            ...
            if (result != null && result.typeOfHit == RayTraceResult.Type.ENTITY && result.entityHit instanceof EntityLivingBase) {
                ...
                if (target instanceof EntityPlayer && target != player) {
                    if (!world.isRemote)
                    {
                        ModularWarfare.NETWORK.sendTo(new PacketHealPart(player, (EntityPlayerMP) target, 0, 3, 0), (EntityPlayerMP) player);
                    }
                }
                else {
                    ModularWarfare.NETWORK.sendTo(new PacketHealPart(player, player, 0, 3, 0), (EntityPlayerMP) player);
                }
