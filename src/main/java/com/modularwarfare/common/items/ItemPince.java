...
public class ItemPince extends Item implements IHasModel {
    public ItemPince(String name)
    {
        ...
        if (entity instanceof EntityPlayer)
        {
            ...
            if (result != null && result.typeOfHit == RayTraceResult.Type.ENTITY && result.entityHit instanceof EntityLivingBase) {
                ...
                if (target instanceof EntityPlayer && target != player) {
                    if (!world.isRemote)
                    {
                        ModularWarfare.NETWORK.sendTo(new PacketHealPart(player, (EntityPlayer) target, 0, 3, 0), (EntityPlayerMP) player);
                    }
                }
                else {
                    ModularWarfare.NETWORK.sendTo(new PacketHealPart(player, (EntityPlayer) player, 0, 3, 0), (EntityPlayerMP) player);
                }
