        ...
        if (gunType.acceptedBullets != null) {
            if (!unload) {
                ...
                if (!offhandedReload) {
                    ...
                    if (entityPlayer.hasCapability(CapabilityExtra.CAPABILITY_ITEM, null)) {
                        extraSlots = entityPlayer.getCapability(CapabilityExtra.CAPABILITY_ITEM, null);
                        slot1 = extraSlots.getStackInSlot(5);
                        if (slot1 != null && slot1.getItem() instanceof ItemBullet) {
                            ItemBullet itemBullet = (ItemBullet) slot1.getItem();
                            for (String bulletName : gunType.acceptedBullets) {
                                if (bulletName.equalsIgnoreCase(itemBullet.baseType.internalName)) {
                                    int count = slot1.getCount();
                                    if (count > highestBulletCount) {
                                        bulletStackToLoad = slot1;
                                        highestBulletCount = count;
                                        bulletStackSlotToLoad = 500;
                                    }
                                }
                            }
                        }
                        slot2 = extraSlots.getStackInSlot(6);
                        if (slot2 != null && slot2.getItem() instanceof ItemBullet) {
                            ItemBullet itemBullet = (ItemBullet) slot2.getItem();
                            for (String bulletName : gunType.acceptedBullets) {
                                if (bulletName.equalsIgnoreCase(itemBullet.baseType.internalName)) {
                                    int count = slot2.getCount();
                                    if (count > highestBulletCount) {
                                        bulletStackToLoad = slot2;
                                        highestBulletCount = count;
                                        bulletStackSlotToLoad = 600;
                                    }
                                }
                            }
                        }
                ...
                if (!entityPlayer.capabilities.isCreativeMode) {
                    if (offhandedReload) {
                        ...
                        if (bulletStackSlotToLoad >= 500) {
                            if (bulletStackSlotToLoad == 500) {
                                if (entityPlayer.hasCapability(CapabilityExtra.CAPABILITY_ITEM, null)) {
                                    extraSlots = entityPlayer.getCapability(CapabilityExtra.CAPABILITY_ITEM, null);
                                    extraSlots.setStackInSlot(5, bulletStackToLoad.getCount() >= 1 ? bulletStackToLoad : ItemStack.EMPTY);
                                }
                            }
                            else if (bulletStackSlotToLoad == 600){
                                if (entityPlayer.hasCapability(CapabilityExtra.CAPABILITY_ITEM, null)) {
                                    extraSlots = entityPlayer.getCapability(CapabilityExtra.CAPABILITY_ITEM, null);
                                    extraSlots.setStackInSlot(6, bulletStackToLoad.getCount() >= 1 ? bulletStackToLoad : ItemStack.EMPTY);
                                }
                            }
                            else {
                                inventory.setInventorySlotContents(bulletStackSlotToLoad, reserve >= 1 ? bulletStackToLoad : ItemStack.EMPTY);
        ...
        if (!unload) {
            ...
            if (!offhandedReload) {
                for (int i = 0; i < inventory.getSizeInventory(); i++) {
                    ...
                    extraSlots = entityPlayer.getCapability(CapabilityExtra.CAPABILITY_ITEM, null);
                    slot1 = extraSlots.getStackInSlot(5);
                    if (slot1 != null && slot1.getItem() instanceof ItemAmmo) {
                        ItemAmmo itemAmmo = (ItemAmmo) slot1.getItem();
                        for (String ammoName : gunType.acceptedAmmo) {
                            if (ammoName.equalsIgnoreCase(itemAmmo.baseType.internalName)) {
                                if (itemAmmo.type.magazineCount > 1) {
                                    for (int j = 1; j < itemAmmo.type.magazineCount + 1; j++) {
                                        int ammoCount = slot1.getTagCompound().getInteger("ammocount" + j);
                                        if (ammoCount > highestAmmoCount) {
                                            ammoStackToLoad = slot1;
                                            ammoStackSlotToLoad = 500;
                                            highestAmmoCount = ammoCount;
                                            multiMagToLoad = j;
                                        }
                                    }
                                } else {
                                    int ammoCount = slot1.getTagCompound().getInteger("ammocount");
                                    if (ammoCount > highestAmmoCount) {
                                        ammoStackToLoad = slot1;
                                        highestAmmoCount = ammoCount;
                                        ammoStackSlotToLoad = 500;
                    ...
                    slot2 = extraSlots.getStackInSlot(6);
                    if (slot2 != null && slot2.getItem() instanceof ItemAmmo) {
                        ItemAmmo itemAmmo = (ItemAmmo) slot2.getItem();
                        for (String ammoName : gunType.acceptedAmmo) {
                            if (ammoName.equalsIgnoreCase(itemAmmo.baseType.internalName)) {
                                if (itemAmmo.type.magazineCount > 1) {
                                    for (int j = 1; j < itemAmmo.type.magazineCount + 1; j++) {
                                        int ammoCount = slot2.getTagCompound().getInteger("ammocount" + j);
                                        if (ammoCount > highestAmmoCount) {
                                            ammoStackToLoad = slot2;
                                            ammoStackSlotToLoad = 600;
                                            highestAmmoCount = ammoCount;
                                            multiMagToLoad = j;
                                        }
                                    }
                                } else {
                                    int ammoCount = slot2.getTagCompound().getInteger("ammocount");
                                    if (ammoCount > highestAmmoCount) {
                                        ammoStackToLoad = slot2;
                                        highestAmmoCount = ammoCount;
                                        ammoStackSlotToLoad = 600;
            ...
            if (ammoStackSlotToLoad != null) {
                if (!entityPlayer.capabilities.isCreativeMode) {
                    ...
                    else if (ammoStackSlotToLoad >= 500) {
                        if (ammoStackSlotToLoad == 500) {
                            if (entityPlayer.hasCapability(CapabilityExtra.CAPABILITY_ITEM, null)) {
                                extraSlots = entityPlayer.getCapability(CapabilityExtra.CAPABILITY_ITEM, null);
                                extraSlots.setStackInSlot(5, ammoStackToLoad.getCount() >= 1 ? ammoStackToLoad : ItemStack.EMPTY);
                            }
                        }
                        else if (ammoStackSlotToLoad == 600){
                            if (entityPlayer.hasCapability(CapabilityExtra.CAPABILITY_ITEM, null)) {
                                extraSlots = entityPlayer.getCapability(CapabilityExtra.CAPABILITY_ITEM, null);
                                extraSlots.setStackInSlot(6, ammoStackToLoad.getCount() >= 1 ? ammoStackToLoad : ItemStack.EMPTY);
                            }
                        }
                    }
