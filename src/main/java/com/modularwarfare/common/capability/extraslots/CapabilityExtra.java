...
public class CapabilityExtra {
    ...
    public static final Capability<IExtraItemHandler> CAPABILITY_ITEM;

    static {
        CAPABILITY_ITEM = null;
    }
    ...
    public static void playerDeath(final LivingDeathEvent event) {
        if(event.getEntity() instanceof EntityPlayer) {
            ...
            for (int i = 0; i < ((IExtraItemHandler) player.getCapability((Capability) CapabilityExtra.CAPABILITY_ITEM, (EnumFacing) null)).getSlots(); i++) {
                final ItemStack extra = ((IExtraItemHandler) player.getCapability((Capability) CapabilityExtra.CAPABILITY_ITEM, (EnumFacing) null)).getStackInSlot(i);
                if (extra.isEmpty()) {
                    return;
                }
                ...
                ((IExtraItemHandler) player.getCapability((Capability) CapabilityExtra.CAPABILITY_ITEM, (EnumFacing) null)).setStackInSlot(i, ItemStack.EMPTY);
            }
        }
    }
    ...
    public static void playerClone(PlayerEvent.Clone event) {
        IExtraItemHandler oldHandler= (IExtraItemHandler) event.getOriginal().getCapability((Capability) CapabilityExtra.CAPABILITY_ITEM, (EnumFacing) null);
        IExtraItemHandler newHandler= (IExtraItemHandler) event.getEntityPlayer().getCapability((Capability) CapabilityExtra.CAPABILITY_ITEM, (EnumFacing) null);
    ...
    public static void sync(final EntityPlayer entity, final Collection<? extends EntityPlayer> receivers) {
        if (entity.hasCapability(CapabilityExtra.CAPABILITY_ITEM, null)) {
            for (int i = 0; i < ((IExtraItemHandler) entity.getCapability((Capability) CapabilityExtra.CAPABILITY_ITEM, (EnumFacing) null)).getSlots(); i++) {
                final PacketSyncExtraSlot msg = new PacketSyncExtraSlot(entity, i, ((IExtraItemHandler) entity.getCapability((Capability) CapabilityExtra.CAPABILITY_ITEM, (EnumFacing) null)).getStackInSlot(i));
                receivers.forEach(p -> {
    ...
    public static class StorageExtraItem implements Capability.IStorage<IExtraItemHandler> {
        @Nullable
