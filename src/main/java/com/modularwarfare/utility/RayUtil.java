...
public class RayUtil {
    ...
    public static float calculateAccuracyServer(final ItemGun item, final EntityLivingBase player) {
        ...
        if (player.hasCapability(CapabilityHealth.CAPABILITY_HEALTH, null)) {
            IHealth Sget = ((IHealth) player.getCapability((Capability) CapabilityHealth.CAPABILITY_HEALTH, null));
            if (Sget.hasBreak("OBB_LEFTHAND")) {
                acc+=25.0f;
            }
            if (Sget.hasBreak("OBB_RIGHTHAND")) {
                acc+=25.0f;
            }
        }
